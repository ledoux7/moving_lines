...
const Base = ({
  ...
  useEffect(() => {
    if (data && data?.transformed && data?.endpoint === 'leaguedashteamstats' && teamId
      ...
      console.log({ minus: data?.transformed });
      // const res = transformFunc(data?.transformed);
      const colsCareAbout = [
        'FGA', 'FG3A', 'FG3_PCT', 'FG_MISS',
      ];
      // let ranked = JSON.parse(JSON.stringify(data?.transformed));
      const calc = JSON.parse(JSON.stringify(data?.transformed)).map(t => {
  ...
  useEffect(() => {
    if (teamStat) {
      const types = ['FGA', 'FG3A', 'FG3_PCT', 'FG_MISS'];
      const cat = [
        '',
        // '_CHANCES',
        // '_CHANCE_PCT',
