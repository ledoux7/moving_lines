...
function createCourt() {
  ...
  // are 50ft sideline to sideline and 94feet baseline to baseline (47ft half court)
  // Forcing at least a 500x470 ratio for the court in order to paint shots appropriately
  let width = 500;
  let height = 0.94 * width;
  ...
  function court(selection) {
    selection.each(function (data) {
      ...
      if (!d3.select(this).selectAll('svg').empty()) {
        ...
        const dashedFoulCircle = svg.append('g').classed('foul-circle dashed', true);
        ...
        svg.append('g').classed('rim', true)
          ...
          .attr('r', 0.75); // regulation rim is 18 inches
        ...
        threePointArea.append('line')
          ...
          .attr('y2', yScale(0));
        // Add key lines
        const keyLines = svg.append('g').classed('key-lines', true);
